...
def test_get_union_types_with_complex_union():
    assert get_union_types(Union[int, str, List[float], Dict[str, Any]]) == (int, str, List[float], Dict[str, Any])
...
def test_type_constrained_list_is_valid_type():
    ...
    assert l.is_valid_type(1) == True
    assert l.is_valid_type('2') == False
...
def test_type_constrained_list_union_type():
    ...
    l.append('2')
    assert l == [1, '2']
    with pytest.raises(TypeError, match=re.escape("Invalid value type: got float instead of Union[int, str]")):
        l.append(3.0)
...
def test_type_constrained_dict_is_valid_key_type():
    ...
    assert d.is_valid_key_type('a') == True
    assert d.is_valid_key_type(1) == True
    assert d.is_valid_key_type(1.0) == False

def test_type_constrained_dict_is_valid_value_type():
    """Test `is_valid_value_type` method."""
    d = TypeConstrainedDict(str, Union[int, str])
    assert d.is_valid_value_type(1) == True
    assert d.is_valid_value_type('a') == True
    assert d.is_valid_value_type(1.0) == False
